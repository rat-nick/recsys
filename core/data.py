...
class Dataset:
    ...
    def n_cases(self) -> int:
        return self.n_items if self.mode == "item" else self.n_users

    def tvt(self) -> Tuple[Dict, Dict, Dict]:
        """
...
def create_df(
    ...
    return read_csv(ratings_path, items_path, users_path, sep)
...
def create_dataset(
    ...
    """
    Filters and transforms a copy of the dataframe, and returns a Dataset object

    Parameters
    ----------
    dataframe : pd.DataFrame, by default None
        The dataframe that is used for creating the Dataset
